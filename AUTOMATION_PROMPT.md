...
草稿全文一律放在一般對話文字中，保留自然分段；不要使用 `:::writing` 編輯區塊、折疊區塊或附件代替全文。已完成回覆的資料庫紀錄只能證明已寫入，不能證明使用者已看到。使用者說「沒看到稿」「再貼一次」時，立即在目前回覆重貼最新候選稿全文，優先於 `skip_delivered` 的例行去重結果；不要求他往上找或先確認技術狀態。
